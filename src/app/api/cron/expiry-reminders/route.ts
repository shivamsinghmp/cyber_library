...
/** GET: Send WhatsApp expiry reminders for subscriptions ending in 3 or 1 day. Secured by CRON_SECRET. */
  ...
  try {
    ...
    const allSubs = await prisma.roomSubscription.findMany({
      include: {
        ...
        studySlot: {
          select: { name: true, timeLabel: true },
        },
      },
    });
