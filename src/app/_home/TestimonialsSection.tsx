        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
          ...
          <motion.div variants={fadeIn} className="relative overflow-hidden rounded-[2.5rem] border bg-white p-10 md:p-14" style={{ borderColor: "var(--border)", boxShadow: "var(--shadow-lg)" }}>
            ...
            <div className="mt-10 flex items-center justify-center gap-6">
              <button type="button" onClick={() => setActive((p) => (p === 0 ? TESTIMONIALS.length - 1 : p - 1))} className="flex h-12 w-12 items-center justify-center rounded-full border transition-all hover:-translate-y-0.5" style={{ borderColor: "var(--border)", background: "white", color: "var(--foreground)" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
              </button>
              <div className="flex gap-2">
                {TESTIMONIALS.map((_, idx) => (
                  <button key={idx} type="button" onClick={() => setActive(idx)} className="h-2.5 rounded-full transition-all duration-500" style={{ width: idx === active ? "2rem" : "0.625rem", background: idx === active ? "var(--accent)" : "var(--border)" }} />
                ))}
              </div>
              <button type="button" onClick={() => setActive((p) => (p === TESTIMONIALS.length - 1 ? 0 : p + 1))} className="flex h-12 w-12 items-center justify-center rounded-full border transition-all hover:-translate-y-0.5" style={{ borderColor: "var(--border)", background: "white", color: "var(--foreground)" }}>
