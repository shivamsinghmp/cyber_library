...
export const passwordSchema = z
  ...
  .max(72, "Password bahut lamba hai (max 72 characters)")
  .refine((p) => /[A-Z]/.test(p), {
    message: "Kam se kam 1 uppercase letter hona chahiye (e.g. A–Z)",
  })
  .refine((p) => /[0-9]/.test(p), {
    message: "Kam se kam 1 number hona chahiye (0–9)",
  })
  .refine((p) => /[^A-Za-z0-9]/.test(p), {
    message: "Kam se kam 1 special character hona chahiye (e.g. !@#$%&*)",
  });
