...
import { KanbanSquare, Search, LogOut, Menu, X } from "lucide-react";

const navLinks = [
  { href: "/admin/pipeline", label: "Pipeline", icon: KanbanSquare },
  { href: "/admin/students", label: "Student Lookup", icon: Search },
];
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white shadow-sm">
      <div className="flex h-12 items-center gap-2 px-3 md:px-4">
        <Link href="/admin/pipeline" className="flex shrink-0 items-center gap-2 mr-2">
          {isExternalLogo
          ...
          <span className="hidden sm:block text-sm font-bold text-gray-900 truncate max-w-[140px]">{siteTitle} CRM</span>
        </Link>
            ...
            return (
              <Link key={link.href} href={link.href}
                className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium transition ${
                  active ? "bg-orange-700 text-white" : "text-gray-600 hover:bg-gray-100 hover:text-gray-800"
                }`}>
