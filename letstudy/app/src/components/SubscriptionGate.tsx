...
// Resolve the current pathname to a module ID (longest-href-match, so
// sub-routes like /dashboard/wallet/buy-coins still map to "wallet").
function moduleIdForPath(pathname: string): string | null {
  let best: { id: string; href: string } | null = null;
  for (const m of STUDENT_MODULES) {
    if (pathname === m.href || pathname.startsWith(m.href + "/")) {
      if (!best || m.href.length > best.href.length) best = { id: m.id, href: m.href };
    }
  }
  return best?.id ?? null;
}
...
export function SubscriptionGate({ children }: { children: React.ReactNode }) {
  ...
  const { loading: subLoading, planType, endDate, trialUsed } = useSubscription();
  ...
  useEffect(() => {
    if (sessionStatus !== "authenticated") { setAccessLoading(false); return; }
    let cancelled = false;
    setAccessLoading(true);
    fetch("/api/dashboard/access", { credentials: "include" })
      .then((r) => r.ok ? r.json() : { allowedModules: [] })
      .then((d: { allowedModules?: string[] }) => {
        if (!cancelled) setAllowedModules(d.allowedModules ?? []);
      })
      .catch(() => { if (!cancelled) setAllowedModules([]); })
      .finally(() => { if (!cancelled) setAccessLoading(false); });
    return () => { cancelled = true; };
  }, [sessionStatus, pathname]);

  const moduleId = moduleIdForPath(pathname);
  // Pages with no matching module (e.g. /dashboard root itself) are always free.
  const allowed = moduleId === null || (allowedModules?.includes(moduleId) ?? true);
  ...
  if (!allowed && !loading && sessionStatus === "authenticated") {
    return (
  ...
  const planLabel = planType === "MONTHLY" ? "monthly" : planType === "YEARLY" ? "yearly" : "trial";
    ...
    <>
      {daysLeft !== null && daysLeft <= 7 && (
        <div className="mx-4 mt-4 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-medium text-amber-700">
          ...
          Your {planLabel} {planType === "TRIAL" ? "" : "subscription "}expires in{" "}
          <strong>{daysLeft} day{daysLeft !== 1 ? "s" : ""}</strong>.{" "}
          <Link href="/pricing" className="ml-1 underline hover:text-amber-900">
            {planType === "TRIAL" ? "Upgrade now" : "Renew now"}
          </Link>
        </div>
      )}
      {children}
