...
import {
  ...
  ChevronDown, LogOut, Menu, X, Home,
} from "lucide-react";
...
// This app is admin-only (staff log in on staff.lstudy.in instead), so the
// module-based filtering that the old shared AdminTopNav did for EMPLOYEE
// accounts is dropped here — every nav item is always visible to ADMIN.
    ...
    <header ref={navRef} className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white shadow-sm">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-3 py-2 max-h-[70vh] overflow-y-auto">
          <Link href="/admin" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-50">
