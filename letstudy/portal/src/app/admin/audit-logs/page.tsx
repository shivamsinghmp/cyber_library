  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 pb-12">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-[var(--wood)]/20 bg-emerald-50 px-4 py-2.5 text-sm font-bold text-emerald-700">
          <Activity className="h-4 w-4 animate-pulse" />
           Live Tracking Active
        </div>
      </div>

      <div className="overflow-x-auto rounded-3xl border border-gray-200 bg-[var(--ink)]/40 shadow-xl backdrop-blur-md">
        {logs.length === 0 ? (
          ...
          <table className="w-full min-w-[800px] text-sm">
            <thead className="bg-[#1a1511]">
              <tr className="border-b border-gray-200 text-left text-[var(--wood)]">
                ...
                <tr key={log.id} className="transition-colors hover:bg-white/[0.02]">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-semibold text-[var(--cream)] flex items-center gap-2">
                      ...
                      {log.user.role === "ADMIN" && (
                         <span className="rounded bg-indigo-50 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-[var(--accent)] border border-[var(--accent)]/20">Admin</span>
                      )}
                    </div>
                    ...
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold ${
                        log.action === "DELETE" || log.action === "REVOKE"
                          ? "bg-red-50 text-red-400 ring-1 ring-inset ring-red-500/20"
                          : log.action === "CREATE" || log.action === "GRANT"
                          ? "bg-emerald-50 text-emerald-400 ring-1 ring-inset ring-emerald-500/20"
                          : log.action === "UPDATE"
                          ? "bg-blue-50 text-blue-400 ring-1 ring-inset ring-blue-500/20"
                          : "bg-gray-100 text-[var(--cream-muted)] ring-1 ring-inset ring-white/20"
                      }`}
                    >
                      {log.action}
                    </span>
