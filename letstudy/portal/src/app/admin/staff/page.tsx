...
import { z } from "zod";
import { Mail, Lock, UserPlus, Users, Activity, Shield, Check, X as XIcon, Clock } from "lucide-react";
...
type StaffUser = { id: string; name: string | null; email: string; role: string; employeeStatus?: string; createdAt: string };
...
export default function AdminStaffPage() {
  ...
  async function setEmployeeStatus(s: StaffUser, employeeStatus: "APPROVED" | "REJECTED") {
    try {
      ...
      if (res.ok) {
        toast.success(employeeStatus === "APPROVED" ? "Staff account approved — they can now log in." : "Staff request rejected.");
        fetchStaff();
      } else {
        const json = await res.json().catch(() => ({}));
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      {/* Pending approval requests — self-registered from staff.lstudy.in */}
      {staff.some((s) => s.employeeStatus === "PENDING") && (
        <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 shadow-xl">
          <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-900">
            <Clock className="h-5 w-5 text-amber-600" /> Pending Approval ({staff.filter((s) => s.employeeStatus === "PENDING").length})
          </h2>
          <p className="mt-1 text-xs text-amber-700">Self-registered from staff.lstudy.in — they can't log in until approved.</p>
          <div className="mt-4 space-y-2">
            {staff.filter((s) => s.employeeStatus === "PENDING").map((s) => (
              <div key={s.id} className="flex items-center justify-between gap-3 rounded-xl border border-amber-200 bg-white px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{s.email} · requested {new Date(s.createdAt).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setEmployeeStatus(s, "REJECTED")}
                  ...
                  <button type="button" onClick={() => setEmployeeStatus(s, "APPROVED")}
                    ...
                    <tr key={s.id} className="border-b border-gray-100">
                      <AdminTd className="text-gray-900">
                        ...
                        {s.employeeStatus === "PENDING" && (
                          <span className="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700">Pending</span>
                        )}
                        {s.employeeStatus === "REJECTED" && (
                          <span className="ml-2 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-700">Rejected</span>
                        )}
                      </AdminTd>
                      <AdminTd className="text-gray-900">{s.email}</AdminTd>
