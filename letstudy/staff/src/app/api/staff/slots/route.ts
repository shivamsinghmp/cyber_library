...
export async function GET() {
  try {
    ...
    const slots = await prisma.studySlot.findMany({ orderBy: { createdAt: "asc" } });
    for (const s of slots) {
      if (!s.roomId) {
        const newRoomId = await generateRoomId();
        await prisma.studySlot.update({ where: { id: s.id }, data: { roomId: newRoomId } });
        (s as { roomId: string }).roomId = newRoomId;
      }
    }
    return NextResponse.json(slots);
  } catch (e) {
    console.error("GET /api/staff/slots:", e);
...
export async function POST(request: Request) {
  try {
    ...
    console.error("POST /api/staff/slots:", e);
    return NextResponse.json({ error: "Failed" }, { status: 500 });
  }
}
