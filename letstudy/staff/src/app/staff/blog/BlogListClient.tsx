...
export default function BlogListClient() {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      const res = await fetch("/api/staff/blog", { credentials: "include" });
  ...
  async function handleDelete(id: string, title: string) {
    ...
    setDeleting(id);
    try {
      const res = await fetch(`/api/staff/blog/${id}`, { method: "DELETE", credentials: "include" });
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-sm">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <Link href="/staff/blog/new" className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--accent)] hover:underline">
