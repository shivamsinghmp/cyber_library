...
import { Check, Star, Zap, Gift, Shield, Users } from "lucide-react";
...
// subdomains) — every visitor here is logged-out by definition, so login,
// trial activation, and checkout all hand off to app.lstudy.in instead of
...
export default function PricingPage() {
  ...
  const perMonth = billing === "yearly" ? Math.round(data.yearly.offerPrice / 12) : data.monthly.offerPrice;
    ...
    <div className="min-h-screen bg-[var(--background)]">
      <div className="relative overflow-hidden bg-[var(--foreground)] px-4 pb-16 pt-14 text-center">
        ...
        <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-400/30 bg-amber-400/10 px-4 py-1.5">
          ...
          <span className="text-xs font-semibold text-amber-300">7-day free trial available — no card needed</span>
        </div>
          ...
          <div className="flex rounded-xl border border-white/10 bg-white/5 p-1">
            <button
              ...
              className={`rounded-lg px-6 py-2 text-sm font-semibold transition-all duration-200 ${
                billing === "monthly" ? "bg-white text-[var(--foreground)] shadow" : "text-white/60 hover:text-white"
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`flex items-center gap-2 rounded-lg px-6 py-2 text-sm font-semibold transition-all duration-200 ${
                billing === "yearly" ? "bg-white text-[var(--foreground)] shadow" : "text-white/60 hover:text-white"
              }`}
            >
              Yearly
              {yearlySaving > 0 && (
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                    billing === "yearly" ? "bg-emerald-100 text-emerald-700" : "bg-emerald-500/20 text-emerald-400"
                  }`}
                >
                  save {yearlySaving}%
                </span>
      ...
      <div className="mx-auto -mt-6 max-w-4xl px-4 pb-16">
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          >
            <div className="mb-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted-text)]">Free</p>
              <div className="mt-2 flex items-baseline gap-1">
                <span className="text-5xl font-black text-[var(--foreground)]">{data.currency}0</span>
            ...
            <a
              href={`${APP_URL}/signup?callbackUrl=${encodeURIComponent("/dashboard")}`}
              className="mb-6 flex w-full items-center justify-center gap-2 rounded-xl border-2 border-amber-400 bg-amber-50 py-3 text-sm font-bold text-amber-700 transition hover:bg-amber-100"
            >
              <Gift className="h-4 w-4" /> Start 7-Day Free Trial
            </a>

            <ul className="space-y-3.5">
              {["Browse course catalog", "Read free blog articles", "Access community forum"].map((text, i) => (
          ...
          >
            ...
            <div className="mb-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-white/50">{data.planName}</p>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <span className="text-5xl font-black text-white">
                    {data.currency}{perMonth.toLocaleString("en-IN")}
                  </span>
            ...
            <a
              href={`${APP_URL}/login?callbackUrl=${encodeURIComponent("/pricing")}`}
              className="mb-2 block w-full rounded-xl bg-white py-3 text-center text-sm font-bold text-[var(--foreground)] shadow transition hover:bg-white/90 active:scale-[0.98]"
            >
              Login to Upgrade
            </a>
