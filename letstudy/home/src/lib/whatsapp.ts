 ...
 *
 * One Meta App, 3 phone numbers: AI_CHAT (the StudyMate bot — now lives in
 * python_server, not sent from here), AUTH (OTP, password reset, welcome
 * message, and the new support line), REPORTS (daily/weekly progress
 * reports). Each line resolves its own access token + phone number ID, with
 * the legacy single WHATSAPP_ACCESS_TOKEN/WHATSAPP_PHONE_NUMBER_ID as a
 * fallback so this doesn't hard-break before both lines are configured.
 */
...
export type WhatsAppLine = "AUTH" | "REPORTS";

const LINE_ENV_KEYS: Record<WhatsAppLine, { phoneId: string; token: string }> = {
  AUTH:    { phoneId: "WHATSAPP_AUTH_PHONE_NUMBER_ID",    token: "WHATSAPP_AUTH_ACCESS_TOKEN" },
  REPORTS: { phoneId: "WHATSAPP_REPORTS_PHONE_NUMBER_ID", token: "WHATSAPP_REPORTS_ACCESS_TOKEN" },
};

async function getCredentials(line: WhatsAppLine = "AUTH") {
  const keys = LINE_ENV_KEYS[line];
  const phoneId =
    process.env[keys.phoneId]?.trim() ||
    (await getAppSetting(keys.phoneId as Parameters<typeof getAppSetting>[0])) ||
    process.env.WHATSAPP_PHONE_NUMBER_ID?.trim() ||
    (await getAppSetting("WHATSAPP_PHONE_NUMBER_ID"));
  const token =
    process.env[keys.token]?.trim() ||
    (await getAppSetting(keys.token as Parameters<typeof getAppSetting>[0])) ||
 ...
 */
export async function sendWhatsAppText(toPhoneNumber: string, text: string, line: WhatsAppLine = "AUTH"): Promise<string | null> {
  const { phoneId, token } = await getCredentials(line);
  if (!phoneId || !token) {
...
export async function sendWelcomeMessage(opts: {
  ...
  // Login lives on the student portal subdomain, not on this home/marketing app
  const loginUrl = (process.env.NEXT_PUBLIC_APP_URL?.trim() || "https://app.lstudy.in") + "/login";
...
export async function sendWhatsAppOtp(toPhoneNumber: string, otp: string): Promise<string | null> {
  ...
  const { phoneId, token } = await getCredentials("AUTH"); // OTP is unambiguously the AUTH line
  if (!phoneId || !token) {
