...
import { Megaphone, PenSquare, MessageSquare, Sparkles, LayoutTemplate, Bot } from "lucide-react";
...
export default function AdminWhatsAppPage() {
  ...
  const [mainTab, setMainTab] = useState<"chat" | "report" | "templates" | "bot">("chat");
  ...
  // Auto-open a conversation when linked here with ?phone=... (used by the
  // "Reply on WhatsApp" link from /admin/support's WhatsApp-sourced tickets).
  useEffect(() => {
    const phone = searchParams.get("phone");
    if (phone) handleSelectPhone(phone);
  ...
  async function handleToggleBlock() {
    ...
    const nextBlocked = !activeContact.whatsappBotBlocked;
    ...
    try {
      const res = await fetch("/api/admin/whatsapp/block", {
        ...
        credentials: "include",
        body: JSON.stringify({ userId: activeContact.user.id, blocked: nextBlocked }),
      });
      if (!res.ok) { toast.error("Failed to update block status"); return; }
      setContacts((prev) =>
        prev.map((c) => (c.phoneNumber === selectedPhone ? { ...c, whatsappBotBlocked: nextBlocked } : c))
      );
      toast.success(nextBlocked ? "User blocked from the bot" : "User unblocked from the bot");
    } catch {
  ...
  async function handleSendMessage(e: React.FormEvent) {
    ...
    setInputText("");
    try {
      // Reply on the same line the conversation is on. Next.js only holds
      // AUTH/REPORTS credentials (AI_CHAT lives in python_server), so a manual
      // reply into an AI_CHAT thread goes out via AUTH instead — a known
      // limitation of admin-intervening in an AI bot conversation.
      const replyLine = activeContact?.line === "REPORTS" ? "REPORTS" : "AUTH";
      const res = await fetch("/api/admin/whatsapp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phoneNumber: selectedPhone, message: textSnapshot, userId: activeContact?.user?.id, line: replyLine }),
      });
      const data = await res.json();
      if (!res.ok) {
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 h-[90vh] flex flex-col">
      ...
      <div className="mb-4 flex gap-1 rounded-xl border border-gray-200 bg-white p-1 self-start">
        {([
          ...
          { id: "bot",       label: "Bot Settings",    icon: <Bot             className="h-3.5 w-3.5" /> },
        ] as const).map((tab) => (
          <button
            key={tab.id}
