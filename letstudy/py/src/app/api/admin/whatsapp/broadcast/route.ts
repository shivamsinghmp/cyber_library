  ...
  try {
    ...
    if (slotId) {
      const subs = await prisma.roomSubscription.findMany({
        where: { studySlotId: slotId, user: { deletedAt: null, profile: { is: { whatsappMarketing: true } } } },
        select: {
          user: {
            select: {
    ...
    const users = await prisma.user.findMany({
      where: {
        role: "STUDENT",
        deletedAt: null,
        profile: { is: { whatsappMarketing: true } },
        OR: [
...
export async function POST(request: Request) {
  try {
    ...
    for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
      ...
      await Promise.all(
        batch.map(async (r) => {
          const wamid = await sendWhatsAppText(r.phoneNumber, message, "AUTH");
