  ...
  try {
    ...
    if (phoneNumber) {
      ...
      const messages = await prisma.whatsAppMessage.findMany({
        ...
        select: { id: true, wamid: true, phoneNumber: true, direction: true, content: true, contentIv: true, status: true, createdAt: true, userId: true, line: true },
      ...
      select: {
        ...
        user: {
          select: {
            ...
            profile: { select: { whatsappBotBlocked: true } },
          },
        },
      },
      take: 1000,
    ...
    return NextResponse.json(
      latestPerPhone.map((msg) => ({
        ...
        line: msg.line,
        user: msg.user ? { id: msg.user.id, name: msg.user.name, image: msg.user.image, email: msg.user.email } : null,
        whatsappBotBlocked: msg.user?.profile?.whatsappBotBlocked ?? false,
      }))
    );
  } catch (e) {
...
const sendSchema = z.object({
  ...
  // Which of our 3 numbers to reply from — defaults to AUTH (the general
  // admin-services line) when the frontend doesn't know the conversation's
  // existing line yet (e.g. a brand-new "New Message" send).
  line:        z.enum(["AUTH", "REPORTS"]).optional().default("AUTH"),
});
...
export async function POST(request: Request) {
  try {
    ...
    const { phoneNumber, message, userId, line } = parsed.data;

    const wamid = await sendWhatsAppText(phoneNumber, message, line);
